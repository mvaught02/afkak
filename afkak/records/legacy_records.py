...
from .abc import IRecord, IRecordBatch, IRecordBatchBuilder, IRecordBuilder
from .default_records import CorruptRecordException
...
import afkak.codec as codecs
from ..common import UnsupportedCodecError
...
class LegacyRecordBase:
    ...
    RECORD_OVERHEAD_V1 = struct.calcsize(
        ">I" "b" "b" "q" "i" "i"  # CRC  # magic  # attributes  # timestamp  # Key length  # Value length
    )
    ...
    def __repr__(self):
        return (
            ...
            " key={!r}, value={!r}, crc={!r})".format(
                self._offset, self._timestamp, self._timestamp_type, self._key, self._value, self._crc
            )
        )


@implementer(IRecordBatchBuilder)
class LegacyRecordBatchBuilder(LegacyRecordBase):
    ...
    def _maybe_compress(self):
        if self._compression_type:
            ...
            self._encode_msg(
                start_pos=0, offset=0, timestamp=0, key=None, value=compressed, attributes=self._compression_type
            )
            return True
        return False
...
class LegacyRecordMetadata(object):
    ...
    def __repr__(self):
        return "LegacyRecordMetadata(offset={!r}, crc={!r}, size={!r}," " timestamp={!r})".format(
            self._offset, self._crc, self._size, self._timestamp
        )


@implementer(IRecordBuilder)
class LegacyRecordBuilder(LegacyRecordBase):
    __slots__ = ("record_buffer", "magic", "compression_type")
    OFFSET_LENGTH = 8

    def __init__(self, magic, compression_type):
        self.magic = magic
        self.compression_type = compression_type
        self.record_buffer = io.BytesIO()

    def append(self, offset, timestamp, key, value):
        if self.magic == 0:
            self.record_buffer.write(
                LegacyRecordBatch.HEADER_STRUCT_V0.pack(offset, 0, 0, self.magic, self.compression_type)
            )
        else:
            self.record_buffer.write(
                LegacyRecordBatch.HEADER_STRUCT_V1.pack(offset, 0, 0, self.magic, self.compression_type, timestamp)
            )

        self._write_key_value_to_buffer(key, value)

    def _write_key_value_to_buffer(self, key, value):
        if key is not None:
            self.record_buffer.write(struct.pack('>i', len(key)))
            self.record_buffer.write(key)
        else:
            self.record_buffer.write(struct.pack('>i', -1))

        if value is not None:
            self.record_buffer.write(struct.pack('>i', len(value)))
            self.record_buffer.write(value)
        else:
            self.record_buffer.write(struct.pack('>i', -1))

    def build(self):
        records = self.record_buffer.getvalue()
        record_length = len(records)

        if self.magic == 0:
            fmt = LegacyRecordBatch.HEADER_STRUCT_V0
        else:
            fmt = LegacyRecordBatch.HEADER_STRUCT_V1

        # Recalculate length and CRC and insert into buffer
        length_and_crc = fmt.pack(
            0, record_length, calc_crc32(records[LegacyRecordBatch.MAGIC_OFFSET :]), self.magic, self.compression_type
        )
        records = records[: self.OFFSET_LENGTH] + length_and_crc + records[LegacyRecordBatch.LOG_OVERHEAD :]
