...
from .abc import IRecords
from .legacy_records import LegacyRecordBatch, LegacyRecordBatchBuilder, LegacyRecordBuilder
from .default_records import DefaultRecordBatch, DefaultRecordBatchBuilder, CorruptRecordException


@implementer(IRecords)
class MemoryRecords:
    ...
    def next_batch(self, _min_slice=MIN_SLICE, _magic_offset=MAGIC_OFFSET):
        ...
        if len(next_slice) < _min_slice:
            raise CorruptRecordException(
                "Record size is less than the minimum record overhead " "({})".format(_min_slice - self.LOG_OVERHEAD)
            )
        self._cache_next()
...
class MemoryRecordsBuilder:
    __slots__ = ("_builder", "_buffer", "_next_offset", "_closed", "_bytes_written")
    ...
    def __init__(self, magic, compression_type):
        ...
        if magic >= 2:
            ...
            self._builder = LegacyRecordBuilder(magic=magic, compression_type=compression_type)
