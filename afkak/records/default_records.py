...
from .abc import IRecord, IRecordBatch, IRecordBatchBuilder
    ...
    def __next__(self):
        if self._next_record_index >= self._num_records:
            if self._pos != len(self._buffer):
                raise CorruptRecordException(
                    "{} unconsumed bytes after all records consumed".format(len(self._buffer) - self._pos)
                )
            raise StopIteration
        try:
            msg = self._read_msg()
    ...
    def __repr__(self):
        return (
            ...
            " key={!r}, value={!r}, headers={!r})".format(
                self._offset, self._timestamp, self._timestamp_type, self._key, self._value, self._headers
            )
        )
...
class DefaultRecordBatchBuilder(DefaultRecordBase):
    ...
    def size_in_bytes(self, offset, timestamp, key, value, headers):
        if self._first_timestamp is not None:
            ...
            timestamp_delta = 0
        size_of_body = (
            1 + size_of_varint(offset) + size_of_varint(timestamp_delta) + self.size_of(key, value, headers)  # Attrs
        )
